...
function Accounts() {
    ...
    const findFormErrors = () => {
        ...
        if (!amountOutstanding || amountOutstanding === "")
            newErrors.amountOutstanding = "Enter the remaining amount!";

        // paymentDate errors
        if (!paymentDate || paymentDate === "")
            newErrors.paymentDate = "Payment Date is missing!";

        // paymentRef errors
        if (!paymentRef || paymentRef === "")
            newErrors.paymentRef = "Payment Reference missing!";
        ...
        // paymentFor errors
        if (!paymentFor || paymentFor === "")
            newErrors.paymentFor = "Payment for!";

        // requisitionDate errors
        if (!requisitionDate || requisitionDate === "")
            newErrors.requisitionDate = "select requisition date!";

        // requisitionRef errors
        if (!requisitionRef || requisitionRef === "")
            newErrors.requisitionRef = "Requisition Reference missing!";
        ...
        if (!requisitionOwner || requisitionOwner === "")
            newErrors.requisitionOwner = "Requisition Owner missing!";

        // approval errors
        if (!approval || approval === "")
            newErrors.approval = "Enter you name to approve!";
    ...
    const invoicesErrors = () => {
        ...
        if (!invoiceNo || invoiceNo === "")
            newErrors.invoiceNo = "Enter invoice number!";
        ...
        // iClient errors
        if (!iClient || iClient === "")
            newErrors.iClient = "Choose invoice date!";
        ...
        if (!rate || rate > 5 || rate < 1)
            newErrors.rate = "must assign a rating between 1 and 5!";
    ...
    const paymentErrors = () => {
        ...
        // amountPaid errors
        if (!pAmountPaid || pAmountPaid === "")
            newErrors.pAmountPaid = "Enter amount paid!";

        // amountOutstanding errors
        if (!pAmountOutstanding || pAmountOutstanding === "")
            newErrors.pAmountOutstanding = "Enter the remaining amount!";
    ...
    const handleSubmitReciept = (e) => {
        ...
        if (Object.keys(newErrors).length > 0) {
            ...
            axios
            .post("https://avcs-platform.herokuapp.com/receipts", this.state)
            .then(() =>
                this.setState(() => ({
                            ...
                            <Card>
                                ...
                                <Accordion.Collapse eventKey="3">
                                    <Card.Body>
                                        <Form
                                            id="form-prospectus"
                                            onSubmit={handleSubmitRequisitions}
                                        >
                                            <h1>Requisitions</h1>
                                            <Form.Row>
                                                <Col>
